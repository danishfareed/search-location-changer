document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function performSearch(query) {
        try {
            const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&limit=5&email=contact@locationchanger.ext`);
    ...
    async function reverseGeocode(lat, lng) {
        try {
            ...
            const response = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&zoom=18&addressdetails=1&email=contact@locationchanger.ext`);
